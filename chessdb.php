...
function setOverrides( $row, &$moves ) {
	...
	if( $row == 'rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR w' )
	{
		$overrides = array( 'b2e2' => 15, 'h2e2' => 15, 'c3c4' => 15, 'g3g4' => 15, 'c0e2' => 15, 'g0e2' => 15,
					...
					/*'b0a2' => -1, 'h0i2' => -1, 'd0e1' => -1, 'f0e1' => -1, 'b2a2' => -1, 'h2i2' => -1, 'b2c2' => -1, 'h2g2'=> -1*/ );
	...
	else if( $row == 'rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR b' )
	{
		$overrides = array( 'b7e7' => 15, 'h7e7' => 15, 'c6c5' => 15, 'g6g5' => 15, 'c9e7' => 15, 'g9e7' => 15,
					...
					/*'b9a7' => -1, 'h9i7' => -1, 'd9e8' => -1, 'f9e8' => -1, 'b7a7' => -1, 'h7i7' => -1, 'b7c7' => -1, 'h7g7'=> -1*/ );
		foreach( $overrides as $key => $value ) {
			if( isset( $moves[$key] ) ) {
				$oldscores[$key] = $moves[$key];
				$moves[$key] = $value;
...
function getMoves( $redis, $row, $banmoves, $update, $mirror, $learn ) {
	...
	if( $update )
	{
		...
		$memcache_obj->pconnect('frontnode', 11211);
		if( !$memcache_obj )
				...
				if( isset( $_REQUEST['move'] ) && !empty( $_REQUEST['move'] ) && count_pieces( $row ) >= 10 && count_attackers( $row ) > 4 ) {
					...
					if( isset( $moves[$move] ) && isset( $_REQUEST['score'] ) ) {
						if( isset( $_REQUEST['token'] ) && !empty( $_REQUEST['token'] ) && $_REQUEST['token'] == hash( 'md5', hash( 'md5', 'ChessDB' . $_SERVER['REMOTE_ADDR'] . $MASTER_PASSWORD ) . $row . $_REQUEST['move'] . $_REQUEST['score'] ) ) {
							...
							list( $statmoves, $variations ) = getMoves( $redis, $row, array(), false, true, false );
...
/*
					{
						$m = new MongoClient('mongodb://frontnode');
						$collection = $m->selectDB('chessdb')->selectCollection('chessdb');
						$collection->remove( array( '_id' => new MongoBinData(hex2bin(ccbfen2hexfen($row))) ) );
						$collection->remove( array( '_id' => new MongoBinData(hex2bin(ccbfen2hexfen(ccbgetLRfen( $row )))) ) );
						$collection->remove( array( '_id' => new MongoBinData(hex2bin(ccbfen2hexfen(ccbgetBWfen( $row )))) ) );
						$collection->remove( array( '_id' => new MongoBinData(hex2bin(ccbfen2hexfen(ccbgetLRBWfen( $row )))) ) );

						$readwrite_queue->wriaelock();
						$collection2 = $$m->selectDB('ccdbqueue')->selectCollection('queuedb');
						$collection2->remove( array( '_id' => new MongoBinData(hex2bin(ccbfen2hexfen($row))) ) );
						$collection2->remove( array( '_id' => new MongoBinData(hex2bin(ccbfen2hexfen(ccbgetLRfen( $row )))) ) );
						$collection2->remove( array( '_id' => new MongoBinData(hex2bin(ccbfen2hexfen(ccbgetBWfen( $row )))) ) );
						$collection2->remove( array( '_id' => new MongoBinData(hex2bin(ccbfen2hexfen(ccbgetLRBWfen( $row )))) ) );
						$readwrite_queue->writeunlock();
					}
*/
					if( !$dtmtb ) {
					...
					if( $querylimit === FALSE || $querylimit < 10000 )
					{
							...
							if( count( $statmoves ) > 0 ) {
								...
								foreach( $statmoves as $record => $score ) {
									...
									if( $score >= $throttle && $score >= getbestthrottle( $maxscore ) ) {
										...
										echo 'move:' . $record . ',score:' . $score . ',rank:2,note:! (' . str_pad( $variations[$record][0], 2, '0', STR_PAD_LEFT ) . '-' . str_pad( $variations[$record][1], 2, '0', STR_PAD_LEFT ) . ')';
									}
									else if( $score >= $throttle ) {
										if( $isfirst || $isLearner ) {
											if( isset( $oldscores[$record] ) )
												$score = $score . ' (' . $oldscores[$record] . ')';
											echo 'move:' . $record . ',score:' . $score . ',rank:1,note:* (' . str_pad( $variations[$record][0], 2, '0', STR_PAD_LEFT ) . '-' . str_pad( $variations[$record][1], 2, '0', STR_PAD_LEFT ) . ')';
										}
										else
										...
										if( $isfirst || $isLearner ) {
											if( isset( $oldscores[$record] ) )
												$score = $score . ' (' . $oldscores[$record] . ')';
											echo 'move:' . $record . ',score:' . $score . ',rank:0,note:? (' . str_pad( $variations[$record][0], 2, '0', STR_PAD_LEFT ) . '-' . str_pad( $variations[$record][1], 2, '0', STR_PAD_LEFT ) . ')';
										}
										else
											unset( $statmoves[$record] );
									}
		...
		if( isset( $_REQUEST['token'] ) && $_REQUEST['token'] == hash( 'md5', 'ChessDB' . $_SERVER['REMOTE_ADDR'] . $MASTER_PASSWORD ) ) {
			...
			if( $readwrite_queue->trywritelock() ) {
				$canskip = $memcache_obj->get( 'QueueEmpty' );
				if( $canskip === FALSE ) {
					$m = new MongoClient('mongodb://frontnode');
					$collection = $m->selectDB('ccdbqueue')->selectCollection('queuedb');
					...
					if( $cursor->count() > 0 ) {
						...
						$memcache_obj->add( 'QueueEmpty', 1, 0, 30 );
					}
					$cursor->reset();
				}
				$readwrite_queue->writeunlock();
		...
		if( isset( $_REQUEST['token'] ) && $_REQUEST['token'] == hash( 'md5', 'ChessDB' . $_SERVER['REMOTE_ADDR'] . $MASTER_PASSWORD ) ) {
			...
			if( $readwrite_sel->trywritelock() ) {
				$canskip = $memcache_obj->get( 'QueueEmpty2' );
				if( $canskip === FALSE ) {
					$m = new MongoClient('mongodb://frontnode');
					$collection = $m->selectDB('ccdbsel')->selectCollection('seldb');
					$cursor = $collection->find()->sort( array( 'p' => -1 ) )->limit(10);
					if( $cursor->count() > 0 ) {
						...
						$memcache_obj->add( 'QueueEmpty2', 1, 0, 30 );
					}
					$cursor->reset();
				}
				$readwrite_sel->writeunlock();
